...
export class NexisProvider implements Provider {
  ...
  constructor({rpcUrl, chainId, apiEndpoint, apiKey, testnet}) {
    this.apiInstance = axios.create({
      baseURL: 'https://evm-testnet.nexisscan.io/api',
    });
    this.testnet = testnet || false;
    this.provider = new ethers.providers.JsonRpcProvider(
      rpcUrl || 'https://evm-test.nexis.network/rpc',
      {chainId: chainId},
    );
  }
  ...
  async estimateGas(transaction) {
    const {from, to, value, decimals} = transaction;
    return await this.provider.estimateGas({
      from: from,
      to,
      value: parseUnits(value.toString(), decimals || 18),
    });
  }
  ...
  async getTransactions(wallet): Promise<Object> {
    ...
    if (wallet.type === ASSET_TYPE_COIN) {
      url += `https://evm-testnet.nexisscan.io/api?module=account&action=txlist&address=${wallet.walletAddress}&startblock=0&endblock=99999999&page=1&offset=10&sort=desc`;
    } else {
      url += `https://evm-testnet.nexisscan.io/api?module=account&action=tokentx&${wallet.contract}&address=${wallet.walletAddress}&page=1&offset=30&startblock=0&endblock=27025780&sort=desc`;
    }
    ...
    if (status === 200) {
      ...
      if (status === '1') {
        for (let i = 0; i < result.length; i++) {
          ...
          item.explore = configProperties.nexis.explore + `tx/${item.hash}`;
